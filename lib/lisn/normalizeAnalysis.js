...
export function normalizeAnalysis(raw) {
  ...
  // Model returned unidentified — pass through cleanly for mismatch detection
  if (raw.entityType === "unidentified") {
    return {
      ...buildDefault(),
      entityType: "unidentified",
      editorial: { quickVerdict: raw.verdict?.text || "", shortText: "", structuralText: "" },
      _unidentified: true,
    };
  }
  ...
  const base = {
    ...
    structuralScore:  safeNum(raw.structuralScore,  null),
    explorationScore: safeNum(raw.explorationScore, null),
    globalScore:      safeNum(raw.globalScore,      null),
    genreScore:       safeNum(raw.genreScore,       null),
    relatedSuggestions: Array.isArray(raw.relatedSuggestions) ? raw.relatedSuggestions : [],
